...
Our first major claim is that latent space statistics are
...
Unlike latent space statistics, extrapolation issues in $\decoding$
and $\encoding$ actually help the residual space statistic detect
faults better. We will see this in two cases: 1) if the change happens
in the residual space, $p_{\delta}(\mbx\g\mbz)\neq p(\mbx\g\mbz)$,
it is clear that the residual space statistics $\E_{\mbz\sim q_{\mbtheta}}\log\decoding$
...
of the residual space statistics have been previously proposed in
...
will likely go undetected if only the latent space statistic
