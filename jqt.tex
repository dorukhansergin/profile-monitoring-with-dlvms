...
\begin{figure}[H]
\includegraphics[width=0.25\textheight]{figs/profile_examples} \caption{A collection of 64 by 64 image profiles taken from a hot steel rolling
process.}
\label{fig:Rolling}
\end{figure}
...
variational autoencoders (VAE), for Phase-II monitoring. All of the
proposed monitoring statistics in those works differ slightly, but
...
PCA.
\item Through extensive experiments, unlike traditional monitoring statistics,
...
\end{itemize}
%
The rest of the paper is organized as follow\textcolor{black}{s. \ref{sec:Background}
...
statistic candidate in the context of VAEs. That is, given a well
trained VAE, in-control samples should have relatively larger log-likelihood
...
To prove this, we link the the $T^{2}$ and $Q$ statistics of PPCA
...
The function $f$ is parametrized by the horizontal center location
...
\]
\item \textbf{Mean shift}: all of the pixels are added an additive disturbance
$\delta$ as in 
\[
f(\mbp;c_{0},r)\leftarrow f(\mbp;c_{0},r)+\delta
\]
\item \textbf{Magnitude shift:} all of the pixels are added a multiplicative
...
in-control samples with.\textbf{ }For each subfigure, plots on the
...
in a large number of applications \parencite{Zhang2019-lu,wang2019systematic,lee2019process}.
...
the results, we point out \ref{fig:Output-of-the} below for the original
...
In this paper, we focus on evaluating Phase-II monitoring statistics
...
feasibility. First, we point out two important issues related to the
latent variables learned by the encoder of VAE, namely, entanglement
and failure to extrapolate. Based on these issues, we argue that latent
variable-based statistics should be discarded altogether with both
conceptual explanations and real experiments. Second, we point out
that the residual-based statistics based on sampling will require
too many samples to be computationally feasible. Finally, we propose
...
To support our claim, our simulation study first demonstrates the
entanglement and lack of extrapolation of latent variable-based statistics
and its inferior Phase-II monitoring performance. It further shows
...
we validate the superiority of our formulation on a real-life case
...
drops and we are left with $R(\mbmu_{\mbphi}(\mbx))$ only. For $ERE_{2}$,
