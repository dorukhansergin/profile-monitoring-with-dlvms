...
\usepackage[style=chicago-authordate,isbn=false,url=false,eprint=false,minbibnames=9, maxbibnames=9,maxcitenames=3,firstinits=true]{biblatex}
\DeclareFieldFormat[article,inbook,book,incollection,inproceedings,patent,thesis,unpublished]{citetitle}{#1}
\DeclareFieldFormat[article,inbook,book,incollection,inproceedings,patent,thesis,unpublished]{title}{#1} 
...
\subsection{Unifying residual space and Latent space Monitoring Statistics for VAE and PCA} 
\label{sec:residual-latent-vae-pca}

In this section, we would like to generalize the definition of the $ \Tsq $ and $ Q $ statistics of the PCA-based monitoring for VAE. This can be achieved by breaking the tractable formulation of \ref{eqn:VAELoss} into two terms: $\E_{\mbz\sim q_{\mbtheta}}\log\decoding$ and $\KL{\encoding}{p(\mbz)}$.
To understand the rationale behind this, we revisit the assumptions of the model described in \ref{eq:gaussian-factorized}.
Let us formally represent an out-of-control distribution as $p_{\delta}(\mbx)\neq p(\mbx)$, where $\delta$ is the change magnitude in the out-of-control distribution.
Since $p(\mbx)=\int p(\mbx\g\mbz)p(\mbz)d\mbz$, we can anticipate two
sources of out-of-control behaviors: a shift in latent distribution
$p_{\delta}(\mbz)\neq\pz$ or a shift in residual distribution
$p_{\delta}(\mbx\g\mbz)\neq p(\mbx\g\mbz)$. Note that various combinations
of these two shifts represent all possible shifts in the process.
...
be accurately modeled by PCA, both terms play an important role 
for process monitoring \parencite{kim2003process}. This also holds for PPCA and will become the $T^{2}$ and $Q$ statistics in the traditional monitoring framework. To prove this, we link the $T^{2}$ and $Q$ statistics of PPCA (see \ref{sec:bckgrnd:ReviewPCA}) using the ELBO framework.
...
\begin{prop}
	...
	and $\mbM=\mbW^{\top}\mbW+\sigma^{2}\mbI$. Then, the two monitoring can be derived as follows:
	\begin{equation}
...
In this section, we first reveal the shortcomings of the previously proposed VAE-based monitoring methodologies we present in \ref{sec:bckgrnd:critique}. We then explain these shortcomings in detail. Finally, building on these shortcomings, we propose our methodology.
